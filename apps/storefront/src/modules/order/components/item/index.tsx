  ...
  return (
    <div className="flex gap-4" data-testid="product-row">
      <div className="relative shrink-0 w-[72px] aspect-[3/4] overflow-hidden bg-[var(--theme-surface)]">
        {item.thumbnail && (
          <Image src={item.thumbnail} alt={item.product_title ?? ""} fill className="object-cover object-center" sizes="72px" />
        )}
      </div>
      <div className="flex flex-1 min-w-0 items-start justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <span className="font-sans text-[10px] text-[var(--theme-text-muted)]" data-testid="product-quantity">
            Qty: {item.quantity}
          </span>
        </div>
