    ...
    <div className="px-8 py-6">
      ...
      <div className="flex flex-col gap-2 max-w-xs ml-auto">
        <div className="flex items-center justify-between font-sans text-[12px]">
          ...
          <span className="text-[var(--theme-text)]">{fmt(order.subtotal)}</span>
        </div>
        {order.discount_total > 0 && (
          <div className="flex items-center justify-between font-sans text-[12px]">
            <span className="text-[var(--theme-text-muted)]">Discount</span>
            <span className="text-emerald-500">− {fmt(order.discount_total)}</span>
          </div>
        )}
        {order.gift_card_total > 0 && (
          <div className="flex items-center justify-between font-sans text-[12px]">
            <span className="text-[var(--theme-text-muted)]">Gift card</span>
            <span className="text-emerald-500">− {fmt(order.gift_card_total)}</span>
          </div>
        )}
        <div className="flex items-center justify-between font-sans text-[12px]">
          <span className="text-[var(--theme-text-muted)]">Shipping</span>
          <span className="text-[var(--theme-text)]">{fmt(order.shipping_total)}</span>
        </div>
        <div className="flex items-center justify-between font-sans text-[12px]">
          <span className="text-[var(--theme-text-muted)]">Taxes</span>
          <span className="text-[var(--theme-text)]">{fmt(order.tax_total)}</span>
        </div>
