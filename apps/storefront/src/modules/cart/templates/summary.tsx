...
const Summary = ({ cart }: SummaryProps) => {
  ...
  const { currency_code, item_subtotal, shipping_subtotal, discount_subtotal, tax_total, total } = cart

  return (
    <div className="flex flex-col gap-6 border border-[var(--theme-border)] p-6 small:p-8">
      <h2 className="font-sans text-[10px] small:text-[12px] uppercase tracking-[4px] text-[var(--theme-text)]">
        Sumar
      </h2>

      <DiscountCode cart={cart} />

      <div className="flex flex-col gap-3 font-sans text-[10px] small:text-[11px] uppercase tracking-[2px]">
      ...
      <div className="border-t border-[var(--theme-border)] pt-4 flex justify-between items-baseline">
        <span className="font-sans text-[10px] small:text-[12px] uppercase tracking-[3px] text-[var(--theme-text-muted)]">Total</span>
        <span
      ...
      <LocalizedClientLink href={"/checkout?step=" + step} data-testid="checkout-button">
        <button className="w-full py-3.5 font-sans text-[10px] small:text-[11px] uppercase tracking-[4px] bg-hunter-gold text-hunter-dark hover:opacity-90 transition-opacity">
          Finalizează comanda
        </button>
      </LocalizedClientLink>

      <LocalizedClientLink href="/store" className="text-center font-sans text-[9px] uppercase tracking-[3px] text-[var(--theme-text-muted)] hover:text-hunter-gold transition-colors">
