...
export default function DesktopQuickAdd({ variants, options, productHandle: _productHandle, onVariantSelect }: Props) {
  ...
  const getDisabledValues = (optionId: string): Set<string> => {
    ...
    if (isColorOpt) {
      const available = new Set(variants.map((v) => vmap(v)[optionId]).filter(Boolean))
      return new Set(allValues.filter((v) => !available.has(v)))
    }
    const others = Object.entries(selected).filter(([id, val]) => id !== optionId && !!val) as [string, string][]
  ...
  const effectiveVariant = useMemo(() => {
    ...
    return variants.find((v) => v.options?.every((o) => selected[o.option_id ?? ""] === o.value)) ?? null
  }, [selected, variants, options])

  const inStock = effectiveVariant
    ? !effectiveVariant.manage_inventory || (effectiveVariant.inventory_quantity ?? 0) > 0 || effectiveVariant.allow_backorder
    : false

  const selectLabel = useMemo(() => {
    const missing = options.find((o) => !selected[o.id ?? ""])
    if (!missing) return "Selectează"
    return COLOR_TITLES.includes(missing.title?.toLowerCase() ?? "") ? "Selectează culoarea" : "Selectează mărimea"
  }, [options, selected])

  const handleOptionClick = (e: React.MouseEvent, optionId: string, value: string) => {
    ...
    if (isColorOpt && toggled) {
      const sizeOpt = options.find((o) => !COLOR_TITLES.includes(o.title?.toLowerCase() ?? ""))
      if (sizeOpt) {
        const available = new Set(
          variants.filter((v) => vmap(v)[optionId] === toggled).map((v) => vmap(v)[sizeOpt.id ?? ""]).filter(Boolean)
        )
    ...
    // notify image change
    if (onVariantSelect) {
      const fullMatch = variants.find((v) => v.options?.every((o) => next[o.option_id ?? ""] === o.value)) ?? null
  ...
  const handleAdd = async (e: React.MouseEvent) => {
    ...
    setAdding(true)
    await addToCart({ variantId: effectiveVariant.id, quantity: 1, countryCode })
    setAdding(false)
    setSelected({})
  }
  ...
  return (
    <div
      className="hidden sm:block absolute inset-x-0 bottom-0 z-30 translate-y-2 opacity-0 pointer-events-none group-hover:translate-y-0 group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300 ease-out bg-[var(--theme-bg)] border border-[var(--theme-border)]"
    ...
    >
      <div className="px-3 pt-3 pb-3 flex flex-col gap-2">
          ...
          return (
            <div key={option.id} className="flex items-center gap-1 flex-wrap">
              {sortedValues.map((v) => {
                ...
                const unavailable = getDisabledValues(option.id ?? "").has(v.value ?? "")
                return (
                  <button
                    key={v.id}
                    onClick={(e) => !unavailable && handleOptionClick(e, option.id ?? "", v.value ?? "")}
        ...
        <button
          onClick={handleAdd}
          disabled={adding || !inStock || (!effectiveVariant && options.length > 0)}
        ...
        >
          {adding ? "Se adaugă…" : !effectiveVariant && options.length > 0 ? selectLabel : !inStock ? "Stoc epuizat" : "Adaugă în coș"}
        </button>
      </div>
    </div>
  )
