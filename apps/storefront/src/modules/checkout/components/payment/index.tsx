  ...
  return (
    <div className={!isOpen && !paymentReady ? "opacity-50 pointer-events-none select-none" : ""}>
      ...
      <div className={isOpen ? "block" : "hidden"}>
        ...
        {paidByGiftcard && (
          <div className="mb-6">
            <span className="font-sans text-[8px] uppercase tracking-[3px] text-[var(--theme-text-muted)] block mb-1">{t("Metodă de plată")}</span>
            <span className="font-serif italic text-[13px] text-[var(--theme-text-muted)]">{t("Card cadou")}</span>
          </div>
        )}
        ...
        >
          {isLoading ? t("Se procesează…") :
            !activeSession && isStripeLike(selectedPaymentMethod)
              ? t("Introdu datele cardului")
      ...
      <div className={isOpen ? "hidden" : "block"}>
        {cart && paymentReady && activeSession ? (
          <div className="flex items-start gap-8">
            ...
            <div className="flex flex-col gap-1">
              <span className="font-sans text-[8px] uppercase tracking-[3px] text-[var(--theme-text-muted)] mb-1">{t("Detalii")}</span>
              <div className="flex items-center gap-2" data-testid="payment-details-summary">
                ...
                <span className="font-serif italic text-[13px] text-[var(--theme-text-muted)]">
                  {isStripeLike(selectedPaymentMethod) && cardBrand ? cardBrand : t("Un pas suplimentar va apărea")}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <span className="font-sans text-[8px] uppercase tracking-[3px] text-[var(--theme-text-muted)] mb-1">{t("Metodă de plată")}</span>
            <span className="font-serif italic text-[13px] text-[var(--theme-text-muted)]" data-testid="payment-method-summary">{t("Card cadou")}</span>
          </div>
        ) : null}
      </div>
